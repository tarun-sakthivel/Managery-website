...
import { Badge } from "@/components/ui/badge";
import { Plus, ArrowUpDown } from "lucide-react";
//import TaskDialog from "@/components/TaskDialog"; // if your TaskDialog is a default export; adjust if named
...
export default function TasksDashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const path = isTeamLeader ? "/tasks" : "/tasks/mine";
        const data = await apiFetch(path, "GET", undefined, token);
        if (!mounted) return;
        setTasks(Array.isArray(data) ? data : []);
      } catch (err: any) {
        toast({ title: "Failed to load tasks", description: err?.message || String(err) });
      } finally {
        if (mounted) setLoading(false);
      }
    })();
    return () => { mounted = false; };
  }, [token, user, isTeamLeader, toast]);

  // filter tasks for display
  let filteredTasks = isTeamLeader ? tasks : tasks.filter((t) => t.owner_id === user?.id);
  ...
  const handleDeleteTask = async (taskId: string) => {
    ...
    try {
      await apiFetch(`/tasks/${taskId}`, "DELETE", undefined, token);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-6 py-8">
        <div className="flex justify-between items-start mb-8">
          ...
          <div className="flex items-center gap-4">
            ...
            {isTeamLeader && (
              <Button onClick={openCreateDialog} className="bg-purple-700 hover:bg-purple-800 text-white px-6 rounded-lg">
                <Plus className="h-4 w-4 mr-2" />
                Create Task
              </Button>
            )}
              ...
              >
                <div className="p-5">
                  ...
                  <div className="space-y-2 text-xs">
                    ...
                    <div className="flex justify-between text-gray-600">
                      <span>Assigned On:</span>
                      <span className="font-medium text-gray-900">{new Date(task.created_at).toLocaleDateString()}</span>
                    </div>
                  </div>
                </div>
              </Card>
      ...
      <TaskDialog open={isDialogOpen} onOpenChange={setIsDialogOpen} task={selectedTask} onSave={handleSaveTask} onDelete={handleDeleteTask} isTeamLeader={isTeamLeader} />
    </div>
  );
}
